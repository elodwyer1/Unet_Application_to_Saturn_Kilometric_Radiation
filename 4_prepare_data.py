...
flxpath = output_data_fp + '/flux_images/'
flx_labels=[f for f in listdir(flxpath) if isfile(join(flxpath, f))]
flxfiles = [output_data_fp + '/flux_images/'+f for f in flx_labels]
polpath= output_data_fp + '/pol_images/'
pol_labels=[f for f in listdir(polpath) if isfile(join(polpath, f))]
polfiles = [output_data_fp + '/pol_images/'+f for f in pol_labels]


#LFE filepaths
flux_lfe_files = [output_data_fp + '/flux_images/fl_img'+str(i).zfill(4)+'.png' for i in range(count)]
#Final List
flux_lfe_files = [i for i in flux_lfe_files if i in flxfiles]

###############################################################
pol_lfe_files=[output_data_fp +'/pol_images/pl_img'+str(i).zfill(4)+'.png' for i in range(count)]
#Final List
pol_lfe_files=[i for i in pol_lfe_files if i in polfiles]


'''______Mask_________'''
maskpath1=output_data_fp +'/mask_images/'
mask_image_label1=[f for f in listdir(maskpath1) if isfile(join(maskpath1, f))]
mask_fp1 = [maskpath1+i for i in mask_image_label1]
mask_files1 = [output_data_fp + '/mask_images/mask_img'+str(i).zfill(4)+'.png' for i in range(count)]
mask_file1=[i for i in mask_files1 if i in mask_fp1]

'''________labels and trajectory________'''
lfe_info = pd.read_csv(output_data_fp + '/ML_total_catalogue_withaug.csv')
...
np.save(output_data_fp + '/test_label.npy', test_label)


#Save frequency channels to numpy array.
file = readsav(input_data_fp + '/SKR_2004_CJ.sav')
frequency_tmp = file['f']
np.save(output_data_fp + '/frequency.npy', frequency_tmp)
