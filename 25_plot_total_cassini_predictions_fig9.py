...
for num in range(3):
    start_pan = start_inds[num]
    end_pan = end_inds[num]
    fig, ax = plt.subplots(2, 2, figsize=(22, 9))
    plt.subplots_adjust(hspace=0.1, wspace=0.45)
    for i in range(2):
        start_ind = start_pan[i]
        end_ind = end_pan[i]
        time_view_start = df.loc[start_ind,'start'] -pd.Timedelta(30, 'minutes')
        time_view_end = df.loc[end_ind,'end'] + pd.Timedelta(30, 'minutes')
        year = datetime.strftime(time_view_start, '%Y')
        if year == '2017':
            file = input_data_fp + '/SKR _2017_001-258_CJ.sav'
        else:
            file = input_data_fp + f'/SKR_{year}_CJ.sav'
        colour_in = get_polygons(polygon_fp, time_view_start, time_view_end)
        #p = round(df.loc[[start_ind, end_ind],'probability'].mean(),2)
        ax[0, i] = plot_flux(ax[0,i], time_view_start, time_view_end, file, colour_in)
        ax[0, i].text(-0.15,0.95, labels[num][i], horizontalalignment='center',verticalalignment='center',transform = ax[0, i].transAxes,fontsize=30, weight='bold')
        ax[0, i].set_title(f'{time_view_start} to {time_view_end}', fontsize=20)
        ax[1, i] = plot_pol(ax[1,i], time_view_start, time_view_end, file, colour_in)
    
    fp_save = output_data_fp + f'/{model_name}/figures/final_results/{num}.png'
