# -*- coding: utf-8 -*-
"""
Created on Sun Nov  6 16:54:26 2022
...
data_start = '2004-01-01T00:00:00'
data_end = '2017-09-15T23:57:00'
start = pd.Timestamp(data_start)
end = pd.Timestamp(data_end)
data_label1 = datetime.strftime(start, '%Y%j')
data_label2 = datetime.strftime(end, '%Y%j')
...
div = 2403360%73
inds_split_s = np.arange(0, 2403360, 73)
inds_split_e = [i + 128 for i in inds_split_s[:-2]]
t_start = t_timestamp[inds_split_s]
t_end = t_timestamp[inds_split_e]
#to make each input array the exact size it needs to be to be inputted into model,
#We need to set the last end time to be outside of the Cassini time range and pad
#with zeroes.
last_end_inds = [i + 128 for i in inds_split_s[32921:]]
last_end = [i + pd.Timedelta(384, 'm') for i in t_start[32921:]]
t_end = np.concatenate([t_end, last_end])
inds_split_e = inds_split_e + last_end_inds
total_df=pd.DataFrame({'start': t_start, 'end':t_end})
'''
####### Calculate trajectory inputs #########
...
len_padding = (inds_split_e[-1]-len(t_timestamp) ) + 1
padding = np.zeros((48, len_padding))
flux = np.concatenate([flux, padding], axis=1)
pol = np.concatenate([pol, padding], axis=1)
'''
total_df=pd.read_csv(path_to_data + "/catalogue.csv", parse_dates=['start','end'])
_index = np.arange(len(inds_split_s))

for day1, day2, i in zip(inds_split_s[32920:], inds_split_e[32920:], _index[32920:]):
    plt.ioff()
    fp_sav = path_to_data
