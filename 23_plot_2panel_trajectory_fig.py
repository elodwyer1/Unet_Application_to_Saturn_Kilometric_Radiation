...
ax.hist(b[:-1], b, weights=h, linewidth=3.5,color='skyblue',label='Non-LFE',histtype='step')
ax.tick_params(axis='both', which='major', labelsize=18)
ax2 = ax.twinx()
ax.tick_params(axis='both', which='major', labelsize=18)
...
for i in range(len(x_)):
    ax3.plot(x_[i], y_[i], color='gray', alpha=0.5)  
for i, j in zip(lfe['start'], lfe['end']):
    traj = traj_data.loc[traj_data['datetime_ut'].between(i, j),:]
    ax3.plot(traj['localtime'], traj['lat'], color='orange', label='LFE')
for i, j in zip(nolfe['start'], nolfe['end']):
    traj = traj_data.loc[traj_data['datetime_ut'].between(i, j),:]
    ax3.plot(traj['localtime'], traj['lat'], color='skyblue', label='Non-LFE')
...
plt.tight_layout()
#plt.show()
plt.savefig(figure_fp + "/traj_fig.png")
